...
class classificationHybridModel(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.pool1(out)
        #out = out.view(out.size(0), -1) # [BYPASSS ADD]
        out = out.reshape(out.size(0), -1) # [BYPASSS DEL]
...
class maskCNNModel(nn.Module):
    def __init__(self, opts):
        ...
        self.lstm = nn.LSTM(
            opts.conv_dim_lstm, #1024 , 128 * fs/bw
            opts.lstm_dim, #400
            batch_first=True,
            bidirectional=True)
    ...
    def forward(self, x):
        ...
        out, _ = self.lstm(out)
        out = F.relu(out)
        # B W 800(2 x lstm_dim)
        out = self.fc1(out) # B W 800 -> B W 600
        out = F.relu(out)
        # fc1_dim = 600
        out = self.fc2(out) # B W 600 -> B W 256

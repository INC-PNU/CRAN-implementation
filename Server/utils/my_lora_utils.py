...
def estimate_symbol_custom(a,title,sf,fs,bw,tresshold = 0):
    ...
    combine = upper_freq + lower_freq
    # Step 5: Find peak (max bin)
    print("Upper freq max value :",np.max(upper_freq))
    print("Lower freq max value :",np.max(lower_freq))
    print("Combine freq max value :",np.max(combine)) 
    symbol = np.argmax(combine)
...
def fft_plot(dechirped):
    fft_out =  np.fft.fftshift(np.fft.fft(dechirped))
    fft_mag = np.abs(fft_out)

    plt.figure(figsize=(10,4))
    plt.plot(fft_mag)
    plt.title("FFT After Dechirping (LoRa Symbol)")
    plt.xlabel("FFT bin")
    plt.show()
    return
...
def calculate_symbol_alliqfile_cropping_technique(data,sf,bw,sample_rate,show=True):
    ...
    dividor = 16 // (sample_rate // bw)
   
    stft_nfft = n_classes  * (sample_rate // bw) #good
    istft_nfft = stft_nfft // (sample_rate // bw) #Good
    stft_window = n_classes // dividor
    istft_window = stft_window // (sample_rate // bw) 
    ...
    for i in range(frames):
        ...
        if (bw == sample_rate):
            ...
            up_chirp_signal = np.exp(1j * 2 * np.pi * (f0 * t + (f1 / ( symbol_time)) * t**2))
            down_chirp_signal = np.conj(up_chirp_signal)
            dechirped = an_data * down_chirp_signal
